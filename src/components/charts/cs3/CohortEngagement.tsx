...
import { useCS3Cohorts } from '../../../data/loaders'

const WEEK_KEYS = ['w0','w1','w2','w3','w4','w5','w6','w7','w8','w9','w10','w11'] as const
const WEEK_LABELS = ['W0','W1','W2','W3','W4','W5','W6','W7','W8','W9','W10','W11']
const CHART_H = 248

export function CohortEngagement() {
  const data = useCS3Cohorts()
  ...
  useEffect(() => {
    ...
    const margin = { top: 10, right: 16, bottom: 36, left: 64 }
    ...
    const x = d3.scaleBand().domain(WEEK_LABELS).range([0, innerW]).padding(0.06)
    const y = d3.scaleBand().domain(cohorts).range([0, innerH]).padding(0.06)
    const colorFn = (v: number) => d3.interpolateRgb(
      theme.isDark ? '#0F172A' : '#ECFDF5',
      '#009E73',
    )(v / 100)

    data.forEach(row => {
      WEEK_KEYS.forEach((wk, i) => {
        const val = row[wk as keyof typeof row] as number
        const cx = x(WEEK_LABELS[i])!
        ...
        if (bw > 24 && bh > 13) {
          g.append('text')
            ...
            .attr('fill', val > 55 ? (theme.isDark ? '#0F172A' : '#FFFFFF') : theme.axis)
            .attr('font-size', '8px').attr('font-family', theme.fontFamily.data)
